...
def driver_link_callback(request):
    if (
        reverse_lazy("admin:dashboard_driver_changelist") in request.path
        or request.path == reverse_lazy("admin:dashboard_driverwithfilters_changelist")
...
def driver_list_link_callback(request):
    if request.path == reverse_lazy("admin:dashboard_driver_changelist"):
        return True

    if str(reverse_lazy("admin:dashboard_driver_changelist")) in request.path:
        return True

    if str(reverse_lazy("admin:dashboard_driverwithfilters_changelist")) in request.path:
        return True
